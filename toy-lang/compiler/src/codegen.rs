...
impl<'ctx> Codegen<'ctx> {
    ...
    fn accecpt_expression(&mut self, expression: &Expr) -> CodegenResult<ExprResult<'ctx>> {
        match *expression {
            ...
            Expr::NumberExpr(ref number_literal) => {
                ...
                Ok(ExprResult::Float(self.context.f64_type().const_float(number_literal.value)))
            }
            Expr::Ident(ref identifier) => {
                ...
                match option_symbol {
                    ...
                    None => {
                        semantic_error!(format!("Can't not using Identifier {:?} before delcarate it.", identifier.name.as_str()).as_str());
                    }
                }
            }
    ...
    fn accecpt_assigment_expression(&mut self, assignment_expr: &AssigmentExpression) -> CodegenResult<ExprResult<'ctx>> {
        /*
    ...
    fn accept_conditional_expression(&mut self, conditional_expr: &ConditionExpression) -> CodegenResult<ExprResult<'ctx>> {
        /*
    ...
    fn accept_unary_expression(&mut self, unary_expr: &UnaryExpression) -> CodegenResult<ExprResult<'ctx>> {
        ...
        Ok(match unary_expr.operator {
            ...
            _ => {
                semantic_error!(format!("Unary ops shouldn't be {:?}, parser has some error ", unary_expr.operator).as_str());
            }
        })
    }
    fn accecpt_call_expression(&mut self, call_expr: &CallExpression) -> CodegenResult<ExprResult<'ctx>> {
        match self.module.get_function(call_expr.callee_name.as_str()){
            ...
            None => {
                semantic_error!(format!("Can't using function {:?} before declarate it", call_expr.callee_name.as_str()).as_str());
            }
        }
    }
}