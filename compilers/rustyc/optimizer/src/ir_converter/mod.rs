...
impl<'a> Converter<'a> {
    ...
    fn accpet_function_declar(&mut self, func_declar: &FunctionDeclaration) {
        ...
        let params_symbol_type: Vec<SymbolType> = func_declar
            .params
            .iter()
    ...
    fn accept_function_def(&mut self, func_def: &FunctionDefinition) {
        ...
        let params_symbol_type: Vec<SymbolType> = func_def
            .params
            .iter()
            .map(|param_declar| self.map_ast_type_to_symbol_type(&param_declar.value_type))
            .collect();
        ...
        func_convert.symbol_table = SymbolTable {
            table_list: vec![Default::default()],
            root_table: Some(&self.symbol_table),
        };
    ...
    fn accept_int_literal(&mut self, int_literal: &IntLiteral) -> Value {
        let value = match int_literal.base {
            IntLiteralBase::Decimal => int_literal.raw_value.parse::<i128>().unwrap(),
            _ => todo!(),
        };
...
fn map_ast_type_to_symbol_type(
    value_type: &ValueType,
    struct_layout_table: &mut StructLayoutTable,
    ...
    match value_type {
        ...
        | ValueType::Float
        | ValueType::Double
        | ValueType::LongDouble => SymbolType::BasicType(match value_type {
            ValueType::Char => IrValueType::U8,
        ...
        ValueType::PointerType(pointer_type) => SymbolType::PointerType(PointerSymbolType {
            level: pointer_type.level,
            pointer_to: match map_ast_type_to_symbol_type(
        ...
        ValueType::ArrayType(array_type) => SymbolType::ArrayType(ArraySymbolType {
            array_of: Box::new(map_ast_type_to_symbol_type(
                &array_type.array_of,
                struct_layout_table,
                struct_size_table,
            )),
                ...
                StructType::Def(def) => {
                    ...
                    for declarator in &def.declarator {
                        layout.insert(
                            ...
                            StructLayoutEntry {
                                offset,
                                data_type: map_ast_type_to_symbol_type(
                                    &declarator.value_type,
                                    struct_layout_table,
                                    struct_size_table,
                                ),
                            },
        ...
        ValueType::FunctionPointer {
            ...
            pointer_to: PointerToSymbolType::FunctionType {
                ...
                return_type: Box::new(map_ast_type_to_symbol_type(
                    return_type.as_ref(),
                    struct_layout_table,
                    struct_size_table,
                )),
