...
use rustyc_frontend::token::{FloatLiteralBase, IntLiteralBase};
...
impl<'a> FunctionCoverter<'a> {
    ...
    fn accept_condition_expr(&mut self, cond_expr: &ConditionalExpression) -> Value {
        ...
        self.function
            .build_brif_inst(test_value, cond_conseq_block, cond_alter_block);
        // conseq
        self.function
    ...
    fn accept_unary_expr(&mut self, unary_expr: &UnaryExpression) -> Value {
        match &unary_expr.ops {
            ...
            UnaryOps::Plus => self.accept_expr_with_value(&unary_expr.expr),
    ...
    fn accept_dereference_unary_expr(&mut self, unary_expr: &UnaryExpression) -> Value {
        ...
        match symbol_type {
            ...
            SymbolType::PointerType(pointer_type) => {
                ...
                self.function
                    .build_load_register_inst(address, offset_0, pointer_to_type)
            }
    ...
    fn accept_update_expr(&mut self, update_expr: &UpdateExpression) -> Value {
        ...
        if update_expr.posfix {
            ...
            let (next_value, next_const, target_type) = self
                .function
                .align_two_base_type_value_to_same_type(value, const_one, None);
            let is_float = match target_type {
                IrValueType::F32 | IrValueType::F64 => true,
                _ => false,
            };
            ...
            self.function
                .build_store_register_inst(prefix_value, reg, const_one, ir_type.clone());
            prefix_value
        }
    }
            ...
            ChainBase::Identifier(name) => {
                let (base_reg, symbol_layout_ref) = match self.symbol_table.get(&name) {
                    ...
                    None => unreachable!(
                        "{}",
                        self.error_map
                            .unreach_chain_expression_identifier_must_being_in_the_symbol_table
                    ),
                };
                base = base_reg;
        ...
        loop {
            match cur {
                ...
                Expression::Identifier(identifier) => {
                    sequence.reverse();
                    return (ChainBase::Identifier(identifier.name.to_string()), sequence);
                }
        ...
        if let Some(symbol) = self.symbol_table.get(id.name.as_ref()) {
            match &symbol.symbol_type {
                SymbolType::BasicType(ir_type) => {
                    let offset = self.function.create_u8_const(0);
                    self.function
                        .build_load_register_inst(symbol.reg, offset, ir_type.clone())
                }
                SymbolType::PointerType(_pointer_type) => {
    ...
    fn accept_float_literal(&mut self, float_literal: &FloatLiteral) -> Value {
        let value = match float_literal.base {
            FloatLiteralBase::Decimal => float_literal.raw_value.parse::<f64>().unwrap(),
            _ => todo!(),
        };
