...
impl GVNPass {
    ...
    fn rewrite_phi_by_cache_table(&mut self, dst: &Value, from: & Vec<(BasicBlock, Value)> ) -> bool {
        ...
        if is_meaning_less {
            let replace_value = self.replaceable_value_table.get(last_value).or(Some(last_value)).unwrap();
            self.replaceable_value_table.insert(dst.clone() , replace_value.clone());
        }
        // check is redundant
        self.rewrite_inst_by_cache_table(&InstructionData::Phi { opcode: OpCode::Phi, dst: dst.clone(), from: from.clone() })
    }
        ...
        match get_right_hand_side_inst_key(inst_data) {
            Some((key, dst_value)) => {
                if let Some(replace_value) = self.cache_inst_table.get(&key) {
                    self.replaceable_value_table.insert(dst_value.clone(), replace_value.clone());
                    return true;
                }else {
