    ...
    fn traversal_all_instruction_to_fill_use_def_table(&mut self, function: &Function) {
        ...
        for (_, bb) in &function.blocks {
            for inst_id in &bb.instructions {
                ...
                match inst {
                    InstructionData::Add { src1, src2, dst, .. }
                    | InstructionData::Sub { src1, src2, dst, .. }
                    | InstructionData::Mul { src1, src2, dst, .. }
                    | InstructionData::Divide { src1, src2, dst, .. }
                    | InstructionData::Reminder { src1, src2, dst, .. }
                    | InstructionData::FAdd { src1, src2, dst, .. }
                    | InstructionData::FSub { src1, src2, dst, .. }
                    | InstructionData::FMul { src1, src2, dst, .. }
                    | InstructionData::FDivide { src1, src2, dst, .. }
                    | InstructionData::FReminder { src1, src2, dst, .. }
                    | InstructionData::BitwiseAnd { src1, src2, dst, .. }
                    | InstructionData::BitwiseOR { src1, src2, dst, .. }
                    | InstructionData::LogicalAnd { src1, src2, dst, .. }
                    | InstructionData::LogicalOR { src1, src2, dst, .. }
                    | InstructionData::ShiftLeft { src1, src2, dst, .. }
                    | InstructionData::ShiftRight { src1, src2, dst, .. }
                    | InstructionData::Icmp { src1, src2, dst, .. }
                    | InstructionData::Fcmp { src1, src2, dst, .. } => {
                        self.add_to_def_table(dst.clone(), DefKind::InternalDef(inst_id.clone()), function);
                        self.add_to_use_table(src1.clone(), inst_id.clone(), function);
                        self.add_to_use_table(src2.clone(), inst_id.clone(), function);
                    }
                    InstructionData::StoreRegister { base, offset, src, .. } => {
                    ...
                    InstructionData::LoadRegister { base, offset, dst, .. } => {
                        self.add_to_def_table(dst.clone(), DefKind::InternalDef(inst_id.clone()), function);
                        self.add_to_use_table(base.clone(), inst_id.clone(), function);
                        self.add_to_use_table(offset.clone(), inst_id.clone(), function);
                    }
                    ...
                    InstructionData::Call { params, dst, .. } => {
                        ...
                        if let Some(value) = dst {
                            self.add_to_def_table(value.clone(), DefKind::InternalDef(inst_id.clone()), function);
                        };
                    }
