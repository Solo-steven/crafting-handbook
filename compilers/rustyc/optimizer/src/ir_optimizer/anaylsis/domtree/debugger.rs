...
use crate::ir_optimizer::print_table_helper::{print_divider, print_header, get_max_block_id_len, sort_block_ids};
...
macro_rules! generate_print_out_dom_set {
    ($name: ident, $property: ident, $table_name: expr) => {
        fn $name(output: &mut String, sorted_blocks: &Vec<BasicBlock>, table: &DomTable, max_len: usize, row_body_len: usize, row_header_len: usize) {
            output.push_str(print_divider(max_len).as_str());
...
impl DebuggerAnaylsis<DomTable> for DomAnaylsier {
    fn debugger(function: &Function, table: &DomTable) -> String {
        ...
        print_out_dom(&mut output, &sorted_blocks, table, max_len, row_body_len, row_header_len);
        print_out_df(&mut output, &sorted_blocks, table, max_len, row_body_len, row_header_len);
        print_out_dom_children(&mut output, &sorted_blocks, table, max_len, row_body_len, row_header_len);
        output
    }
}