...
fn get_c_code_folder_path() -> String {
    String::from(
        get_assets_folder_root_path().join("c/ir_convert/fun-call/").as_os_str().to_str().unwrap()
    )
}
/// Gte a path the ir result `./test/ir_convert`.
fn get_ir_test_result_folder_path() -> String {
    String::from(
        get_tests_folder_root_path().join("ir_convert/fun-call/").as_os_str().to_str().unwrap()
    )
}
generate_converter_cases!(
    ...
    (pointer_type_basic_type_return, "pointer_type_basic_type_return"),
    (struct_type_return_expr, "struct_type_return_expr"),
    (void_return_expr, "void_return_expr"),
    (call_by_function_pointer_in_structure, "call_by_function_pointer_in_structure"),
