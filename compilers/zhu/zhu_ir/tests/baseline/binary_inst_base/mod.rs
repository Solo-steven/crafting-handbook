...
pub fn build_module() -> Module {
    ...
    let func_id = module.declar_function("binary_inst_base");
    ...
    let reg2 = builder.add_inst([reg1, reg0]);
    let reg3 = builder.sub_inst([reg2, reg1]);
    let reg4 = builder.mul_inst([reg3, reg2]);
    let reg5 = builder.divide_inst([reg4, reg3]);
    let _reg6 = builder.reminder_inst([reg5, reg4]);
