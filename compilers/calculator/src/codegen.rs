...
impl<'ctx> Codegen<'ctx> {
    ...
    pub fn execute(&mut self) -> CodegenResult<f64> {
        let engine = self
            .module
        ...
        match main_func {
            ...
            None => {
                semantic_error!("Must have a main function as entry point");
            }
        }
    }
    ...
    fn accecpt_program_item(&mut self, program_item: &ProgramItem) -> CodegenResult<()> {
        match *program_item {
    ...
    fn accecpt_declaration(&mut self, declaration: &Decl) -> CodegenResult<()> {
        match *declaration {
                ...
                */
                ...
                builder.build_store(
                    ...
                    match variable_declaration.init {
                        Some(ref expr_ref) => match self.accecpt_expression(expr_ref)? {
                ...
                */
                ...
                match self.module.get_function(fun_name) {
                    Some(_) => {
                        ...
                        semantic_error!(format!("function {:?} already exist.", fun_name).as_str());
                    }
                    None => {}
                };
    ...
    fn accecpt_statement(&mut self, statement: &Stmt) -> CodegenResult<()> {
        match *statement {
            ...
            Stmt::IfStmt(ref if_statement) => {
                ...
                let builder = self.context.create_builder();
                builder.position_at_end(llvm_basic_block);
                let test_llvm_value = builder.build_float_compare(
                    FloatPredicate::OGT,
                    test_expr_llvm_value,
                ...
                let alter_llvm_basic_block = match &if_statement.alter {
                    Some(alter_statement) => {
                        let temp_alter_llvm_basic_block = self
                            .context
                            .insert_basic_block_after(conseq_llvm_basic_block, "tmpAlter");
            ...
            Stmt::WhileStmt(ref while_statement) => {
                ...
                */
                ...
                let test_llvm_value = test_builder.build_float_compare(
                    FloatPredicate::OGT,
                    test_expr_llvm_value,
                    self.context.f64_type().const_float(0.0),
                    "tmpCompare",
    ...
    fn accecpt_expression(&mut self, expression: &Expr) -> CodegenResult<ExprResult<'ctx>> {
        match *expression {
            ...
            Expr::NumberExpr(ref number_literal) => {
                ...
                Ok(ExprResult::Float(
                    self.context.f64_type().const_float(number_literal.value),
                ))
            }
                ...
                match option_symbol {
                    ...
                    None => {
                        semantic_error!(format!(
                            "Can't not using Identifier {:?} before delcarate it.",
            ...
            Expr::CallExpr(ref call_expr) => self.accecpt_call_expression(call_expr),
        }
    }
    fn accecpt_assigment_expression(
        &mut self,
        assignment_expr: &AssigmentExpression,
        ...
        */
        ...
        let lhs_symbol = match *assignment_expr.left.as_ref() {
            ...
            _ => {
                semantic_error!("Invalid Left value, Left value should be assignable.");
            }
        };
        let llvm_basic_block = self.current_block.as_ref().unwrap();
        let builder = self.context.create_builder();
        ...
        Ok(ExprResult::Float(rhs))
    }
    fn accept_conditional_expression(
        &mut self,
        conditional_expr: &ConditionExpression,
    ) -> CodegenResult<ExprResult<'ctx>> {
        /*
        ...
        */
        ...
        phi_node.add_incoming(&[
            (&conseq_llvm_value, conseq_llvm_basic_block),
            (&alter_llvm_value, alter_llvm_basic_block),
        ]);
        self.current_block = Some(final_llvm_basic_block);
    ...
    fn accept_unary_expression(
        ...
        */
        ...
        Ok(match unary_expr.operator {
            Operator::Plus => ExprResult::Float(llvm_value),
            Operator::Minus => {
                let llvm_basic_block = self.current_block.as_ref().unwrap();
                let builder = self.context.create_builder();
                builder.position_at_end(*llvm_basic_block);
            ...
            _ => {
                semantic_error!(format!(
                    "Unary ops shouldn't be {:?}, parser has some error ",
                    unary_expr.operator
                )
                .as_str());
            }
        ...
        match self.module.get_function(call_expr.callee_name.as_str()) {
            Some(llvm_funtion_value) => {
                ...
                for param in &call_expr.params {
                    llvm_params_value.push(match self.accecpt_expression(param)? {
                ...
                Ok(ExprResult::BasicEnum(
                    builder
                        .build_call(llvm_funtion_value, llvm_params_value.as_slice(), "tmpCall")
                        .try_as_basic_value()
                        .left()
                        .unwrap(),
                ))
            ...
            None => {
                semantic_error!(format!(
                    "Can't using function {:?} before declarate it",
                    call_expr.callee_name.as_str()
                )
                .as_str());
            }
        }
