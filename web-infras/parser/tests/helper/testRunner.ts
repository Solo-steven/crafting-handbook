...
import { PassTestCaseResult, FailedTestCasesResult,SkipTestCaseResult,  ExpectPassTestCase, ExpectFailedTestCase, TestSuite, TestResult  } from "./type";
...
async function updateTestCase(testCase: ExpectPassTestCase, passResult: Array<PassTestCaseResult>, failedResult: Array<FailedTestCasesResult> ) {
    const codeBuffer = await readFile(testCase.jsFilePath);
    const resultASTString = tryParseCodeStringIntoASTString(codeBuffer.toString());
    if(resultASTString === null) {
        failedResult.push({
...
export async function runTestSuit(testSuite: TestSuite, isUpdate: boolean): Promise<TestResult> {
    ...
    await Promise.all(isUpdate ? [ 
        ...expectPassTestCases.map(testCase => updateTestCase(testCase, passResult, failedResult)),
    ]: [
        ...expectPassTestCases.map(testCase => runExpectPassTestCase(testCase, passResult, failedResult, skipResult)),
        ...expectFailedTestCases.map(testCase => runExpectFailedTestCase(testCase, passResult, failedResult))
    ]);
    return {
        passResult,
        failedResult,
        skipResult: isUpdate ? expectFailedTestCases.map(testCase => ({ fileId: testCase.fileId, filePath: testCase.jsFilePath })) : skipResult
    }
}