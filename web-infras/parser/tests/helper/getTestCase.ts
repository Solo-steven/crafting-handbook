 ...
 * @param testCase 
 */
async function recursivelyFindTestCase(jsDirPath: string, jsonDirPath: string, expectPassTestCases: Array<ExpectPassTestCase>, expectFailedTestCases: Array<ExpectFailedTestCase>  ) {
    ...
    for(const jsFileName of jsFilesInJsDir) {
        ...
        if(isDir) {
            ...
            subDir.push(recursivelyFindTestCase(nextJsDirPath, nextJsonDirPath, expectPassTestCases, expectFailedTestCases));
            continue;
        }
...
export async function getTestSuite(): Promise<TestSuite> {
    ...
    await Promise.all(
        [
            recursivelyFindTestCase(modelCheckingJsRoot, modelCheckingJsonRoot, expectPassTestCases, expectFailedTestCases),
